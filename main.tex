...
\paragraph{}
We can use the following equivalent formulation in order to get rid of the non differentiable equality constraints functions : 

\begin{eqnarray*}
&\min_{(z^{+}, z^{-}) \in \mathbb{R}^{2m}}& (z^{+} - z^{-})^T (z^{+} - z^{-})\\
subject~~to~~:~~&B(z^{+} - z^{-}) = y& \\
&\forall i \in \llbracket 1, m \rrbracket , & z^{+}_i + z^{-}_i \geq \kappa \\
&\forall i \in \llbracket 1, m \rrbracket , & z^{+}_i z^{-}_i = 0 \\
& z^{+} \geq 0 & \\
& z^{-} \geq 0 &\\
\end{eqnarray*}
