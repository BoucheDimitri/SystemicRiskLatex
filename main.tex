...
Reaaranging the terms, the condition condition is:
...
If no bank has defaulted in $t-1$, we have that $\wh L^i_t = L^i_{t-1}$ and $ \wh D^i_t = D^i_{t-1}$, and thus we have:
...
\begin{itemize}
    ...
    \item $\widehat{P}_t^i + \widehat{R}_t^i - r(D_t^i - L_t^i) > \alpha^i \mathcal{A}_t^i$. In that case, the bank sells enough portfolio assets to comply with the financial choice. Which is enough to comply also with the reserve rule since we have: $\widehat{P}_t^i - \alpha^i \mathcal{A}_t^i > - \widehat{R}_t^i + r(D_t^i - L_t^i) $. As a consequence: 
    $$ P_t^i = \alpha^i \mathcal{A}_t^i.$$
