...
\othernote{Mettre des points à la fin des équations lorsqu'elles finissent des phrases}
...
\othernote{Mettre des chapeaux pour toutes les variables. }

\othernote{Definition : a bank defaults when...}

\item \textbf{Stage 2 : checking for default}. Banks for which $E_t^i \leq \bar{E}^i$ declare
default and are liquidated (see section \ref{dynamic with bankruptcy} for the detailed processes of liquidation). Although their defaulting is not public information yet, it will become so at the beginning of the next period. The vector $\bar{E}$ which components are the $\bar{E}^i$ is a minimal threshold value for the equity of each bank. It enables us to implicitly include deposits from investors or individuals from outside the system in the balance sheets of banks.

\othernote{On sait d'ores et déjà que D, L et E au temps t sont les valeurs chapeaux, mais d'un point de vue formalisme mieux vaut garder les chapeaux pour toutes les variables par soucis de clareté}
...
\paragraph{}
Applying expectation conditionally on $\mathcal{F}_t$ on to (\ref{eq:eqtdyna}) we get : 
...
Although this condition may prove too difficult to enforce at each step. We will thus consider only its initial version :
...
We can thus conclude that a bank that has not defaulted in $t$ always has enough liquidity to be able to reallocate its liquid assets in $t$ so as to comply with the reserve rule.

Thus, this rule enables us to avoid liquidity defaults, and being solvent is a sufficient condition to be able to comply with it. This is coherent with the fact that we want to model only fundamental default. Default can be defined as either having not enough cash to honor one's financial obligation (liquidity default) or not having enough equity (solvency default)---or both. We have shown here that since the only illiquid assets are inter-bank loans, if equity is positive in $t$ then by construction liquidity default cannot happen in $t$. 
...
The evolution of reserves are as follows : 

\begin{equation}\label{eq:rsv}
\widehat{R}_t^i = R_{t-1}^i + rL_t^i - rD_t^i
\end{equation}
...
E^i_t = \widehat{R}^i_t + L^i_t + \widehat{P}_t^i - D^i_t
\end{equation}

Putting all above dynamic equation together yields: 

$$ E^i_t = R_{t-1}^i + L^i_t - D^i_t + P_{t-1}^i + rL_t^i - rD_t^i + Q_{t-1}^i\Delta X_t$$

If no bank has defaulted in $t-1$, we have that $L^i_t = L^i_{t-1}$ and $D^i_t = D^i_{t-1}$, and thus we have :

$$E^i_t = R_{t-1}^i + L^i_{t-1} + P_{t-1}^{i} - D^i_{t-1} + rL_t^i- rD_t^i + Q_{t-1}^i\Delta X_t$$
...
\Leftrightarrow E^i_t = E^i_{t-1} + rL_{t-1}^i - rD_{t-1}^i + Q_{t-1}^i\Delta X_t
\end{equation}
...
\begin{itemize}
    \item $\widehat{R}_t^i < r(D_t^i - L_t^i)$ (reserve rule not matched)
    \item $\widehat{R}_t^i \geq r(D_t^i - L_t^i)$ (reserve rule matched)
\end{itemize}
...
\begin{itemize}
    \item $\widehat{P}_t^i > \alpha^i \mathcal{A}_t^i$. In that case, a portion $\widehat{P}_t^i - \alpha^i \mathcal{A}_t^i$ of the portfolio must be sold. No other actions are required, thus $$P_t^i = \alpha^i \mathcal{A}_t^i$$
    \item $\widehat{P}_t^i \leq \alpha^i \mathcal{A}_t^i$. In that case the bank wants to increase its portfolio to saturate the constraint if possible given the reserve rule. Thus $$P_t^{i} = \min \left(\alpha^i \mathcal{A}_t^i,~~\widehat{P}_t^i + \widehat{R}_t^i - r(D_t^i - L_t^i) \right)$$
\end{itemize}
...
\begin{itemize}
    \item $\widehat{P}_t^i + \widehat{R}_t^i - r(D_t^i - L_t^i) \leq \alpha^i \mathcal{A}_t^i$. In that case, the bank sells only the amount necessary to comply with the reserve rule --- since $\widehat{R}_t^i - r(D_t^i - L_t^i) < 0$ this is indeed selling . As a consequence : 
    $$ P_t^{i} = \widehat{P}_t^i + \widehat{R}_t^i - r(D_t^i - L_t^i)$$
    \item $\widehat{P}_t^i + \widehat{R}_t^i - r(D_t^i - L_t^i) > \alpha^i \mathcal{A}_t^i$. In that case, the bank sells enough to comply with the financial choice which is necessarily enough to comply also with the reserve rule. As a consequence : 
...
P_t^i = \min(\widehat{P}_t^i + \widehat{R}_t^i - r(D_t^i - L_t^i),~~\alpha^i \mathcal{A}_t^i)
\end{equation}
...
Injecting (\ref{rebal_synthesis}) into (\ref{quantities_bij}), we deduce the new quantities :
\begin{equation}\label{eq:rebal_quantities}
\forall l,~~ Q_t^{il} = Q_{t-1}^{il}\left(1 + \frac{\min(\widehat{P}_t^i + \widehat{R}_t^i - r(D_t^i - L_t^i),~~\alpha^i \mathcal{A}_t^i) - \widehat{P}_t^i}{\widehat{P}_t^i} \right)
\end{equation}
...
$$\Psi_t^{ij} = \frac{L_t^{ij}}{\sum_{i'=1}^n L_t^{i'j}} $$
...
\paragraph{}
In the internal settlement case, liquidation follows the procedure :
\othernote{Faire un paragraphe d'introduction avec du texte sans les équations pour mieux comprendre : on vend tous les actifs liquides et on transfère les prêts des banques en défaut sont distribuées proportionnellement à ses créditeurs}
 \begin{enumerate}
    ...
    \begin{itemize}
        ...
        $$\forall i \notin \mathcal{D}_{0:t-1},~~\forall k \notin \mathcal{D}_{0:t-1},  ~~L_t^{ik} = L_{t-1}^{ik} + \Psi_{t-1}^{ij} L_{t-1}^{jk}$$
        ...
        \begin{eqnarray*}
        \forall i,~~L_t^i = \sum_{s=1}^nL_t^{is} \\
        \forall i,~~D_t^i = \sum_{s=1}^nL_t^{si}
        \end{eqnarray*}
        ...
        $$ \widehat{R}_t^i = R_{t-1}^i + r (L_t^i - D_t^i) + \Psi_{t-1}^{ij} \pi_{t-1}^j$$
        Let us highlight that since the graph is rewired the amount of interests paid on loans by each non defaulting bank is not affected by the defaults of other banks.
...
When a bank $j$ goes bankrupt at time $t-1$, the liquidator buys the totality of the bankrupt bank's loans to other banks with a discount $0<\zeta<1$. For the rest the procedure is similar to the previous one :
 ...
 \begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{eqnarray*}
        R_t^0 &=& R_{t-1}^0 - \zeta L^j_{t-1}\\
        L_t^0 &=& L_{t-1}^0 + L^j_{t-1}\\
        E_t^0 &=& E_{t-1}^0 + (1 -\zeta)L^j_{t-1}\\
        \forall k\neq 0,~~L_t^{0k} &=& L_{t-1}^{0k} + L_{t-1}^{jk}\\
        \end{eqnarray*}
        ...
        \item Proceedings of liquidation are distributed to the creditors of $j$ proportionally to their claim on $j$. This implies a modified version of the equation (\ref{eq:rsv}): 
        $$ \widehat{R}_t^i = R_{t-1}^i + r L_t^i - rD_t^i + \Psi_{t-1}^{ij} \pi_{t-1}^j$$
        Let us highlight that since the graph is rewired the amount of interests paid on loans by each non defaulting bank is not affected by the defaults of other banks.
...
\section{Synthesis of the model}
\othernote{Ecrire les équations avec des fonctions indicatrices lorsque l'on a besoin de disjoindre des cas selon que les banques ont fait défaut ou non. Somme sur j dans les banques ayant fait défaut.}

\othernote{Faire deux synthèses, une pour le liquidateur, une pour internal settlement}

\othernote{Passer en annexe, on répète ce qui a été dit sans rajouter quoique ce soit, c'est plus pour nous et pour le codage qu'autre chose}

\othernote{Commencer à mettre des quantificateurs pour décrire ce qui se passe pour tout le réseau}

\othernote{Update equity : développer la formule pour montrer les trois types de sources potentielles de pertes : taux d'intérêt, portfolio, défaut}
...
\begin{itemize}
    ...
    $$\omega_t^l = \frac{X_t^l - X_{t-1}^l}{X_{t-1}^l}$$
...
$j$ is the generic index for a bank that has defaulted in period $t-1$
\begin{itemize}
    ...
    $$ \widehat{R}_t^i = R_{t-1}^i + rL_t^i - rD_t^i + \Psi_{t-1}^{ij} \pi_{t-1}^j$$
...
\begin{itemize}
    \item Check if no new bank $j$  has defaulted :
    $$\widehat{E}_t^j \leq \bar{E}^j $$
    \begin{itemize}
        \item If that is the case carry on
        \item Else, $\pi_t^j = 0$ and $\Psi_t^{ij}=0$ and jump to stage 3
    \end{itemize}
    
    \item Computation of the proceedings of liquidition
    $$\pi_t^j = \xi \widehat{P}_t^j + \widehat{R}_t^j + \zeta L^j_t $$ 
...
$$P_t^{j} = \min(\widehat{P}_t^j + \widehat{R}_t^j - r(D_t^j - L_t^j),~~\alpha^j \mathcal{A}_t^j)$$

\item Update the quantities consequently
$$\forall l,~~ Q_t^{jl} = Q_{t-1}^{jl}\left(1 + \frac{\min(\widehat{P}_t^j + \widehat{R}_t^j - r(D_t^j - L_t^j),~~\alpha^j \mathcal{A}_t^j) - \widehat{P}_t^j}{\widehat{P}_t^j} \right)$$

\item Set the definitive reserves for $t+1$ accordingly
$$R_t^j = \widehat{R}_t^j + \widehat{P}_t^j - P_t^{j}$$
\end{itemize}
